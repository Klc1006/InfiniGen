...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id, max_num_kv=400):
        self.config = config
        self.env = env
    ...
    def set_task(self, task):
        ...
        if self.layer_id > 1:
            ...
            self.hh_k = int(task.prompt_len * 0.5)
    ...
    def init_cache_one_gpu_batch(self, cache_home):
        ...
        if self.layer_id > 1:
            ...
            self.prefetch_kv = device.allocate((2, 2048, cache_home.val[0].shape[1], cache_home.val[0].shape[2]), np.float16, pin_memory=True)
    ...
    def load_cache(self, cache_home, cache_read_buf, i):
        ...
        if path == 0:  # Direct copy
            ...
            if self.policy.attn_sparsity >= 1.0:
                self.prefetch_kv.data[0, :pos] = k_home.data[:pos]
                self.prefetch_kv.data[1, :pos] = v_home.data[:pos]
                k_c = TorchTensor((pos, k_home.shape[1], k_home.shape[2]), k_home.dtype, self.prefetch_kv.data[0, :pos], k_home.device)
                v_c = TorchTensor((pos, v_home.shape[1], v_home.shape[2]), v_home.dtype, self.prefetch_kv.data[1, :pos], v_home.device)
...
class OptLM:
    def __init__(self,
        ...
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer:
                layers.append(SelfAttention(self.config, self.env, self.policy, i, max_num_kv))
    ...
    def generation_loop_normal(self):
        for i in range(self.execute_gen_len):
            ...
            for j in range(self.num_layers):
                ...
                for k in range(self.num_gpu_batches):
                    if (j in self.attn_layer[:2]) and (i > 0):
                    ...
                    if (j in self.attn_layer[1:-1]) and (i > 0):
                        self.load_cache(i, self.attn_layer[self.attn_layer.index(j) + 1], k, overlap=True)
...
def run_flexgen(args):
    ...
    model = OptLM(opt_config, env, args.path, policy, args.max_num_kv)
